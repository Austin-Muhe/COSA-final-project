...
COSA adapts forecasts in the output space by using recent test-time context rather than updating the full forecasting backbone \cite{cosa_repo}. This makes it attractive for deployment because it is lightweight and can be attached to existing forecasting models. We use DLinear as the backbone because it is simple, efficient, and a strong baseline for long-term forecasting \cite{zeng2023dlinear}. The datasets follow common long-term forecasting benchmarks: ETTh1 from the ETT benchmark \cite{ett} and Weather from standard multivariate forecasting evaluations.

This project also relates to robustness under distribution shift. Standard train/validation/test evaluation usually assumes a stable test distribution. Our black-swan tests instead inject abrupt shifts into the test stream and measure the immediate post-shift error. This setting evaluates whether recent context helps adaptation or becomes misleading near a shift boundary.
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
where $S$ is one of four synthetic shifts: level, variance, trend, or spike. We report \texttt{mse\_after\_50}, the MSE after a short post-shift warm-up, to focus on recovery rather than only the exact boundary.

\section{Implementation Details}
The official implementation is kept in \texttt{external/COSA\_ICLR2026/}. Project code is separated into \texttt{experiments/}, \texttt{experiments/tta/}, \texttt{experiments/blackswan/}, and \texttt{results/}. The main entry point is \texttt{experiments/run\_cosa\_plus.py}; black-swan shifts are implemented in \texttt{experiments/blackswan/shift\_injector.py} and \texttt{experiments/blackswan/run\_blackswan.py}. Final tables and figures are stored in \texttt{results/final\_figures/}.
...
Weather gives a more cautious conclusion. Adaptation is clearly valuable under shifts: original COSA reduces post-shift MSE by about 8\% on variance shift and more than 20\% on trend/spike shifts compared with no-TTA. However, the full COSA+ variant is slightly worse than original COSA. The rich-context-only variant is almost tied with original and slightly best on variance/spike. This suggests that volatility-aware context is safe, while combining it with a vector gate needs stronger regularization or dataset-specific tuning.

\section{Conclusion}
This project reproduced COSA and explored COSA+ for robust test-time adaptation in time-series forecasting. The main finding is conditional rather than absolute. COSA+ does not reliably improve clean short-horizon performance, and on Weather the full vector gate can hurt. However, at ETTh1 horizon 720 under abrupt black-swan shifts, COSA+ consistently improves over original COSA and no-TTA. The result supports the idea that richer output-space adaptation is most useful when the forecast horizon is long and post-shift errors accumulate over many future steps. Future work should tune the gate regularization, add explicit shift detection, and test more backbones such as iTransformer.
...
Main scripts used in this project include \texttt{experiments/run\_05\_main\_exp.sh}, \texttt{experiments/run\_04\_ablation.sh}, \texttt{experiments/run\_06\_blackswan\_mag3.sh}, and \texttt{experiments/make\_final\_artifacts.py}. The final artifacts used by this report are in \texttt{results/final\_figures/}.
