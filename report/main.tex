...
Black Swan metric & \texttt{mse\_after\_50} for COSA-family robustness checks \\
...
\FloatBarrier
\subsection{Supporting COSA-Family Robustness Check}
The abrupt-shift experiments contextualize both online adaptation and zero-gradient adaptation. On ETTh1/DLinear/horizon-720, Table~\ref{tab:blackswan-main} shows that the meta-trained Dynamic Regime Adapter is strongest on level, trend, and spike shifts, while COSA+ remains slightly best on variance shift. The untrained dynamic adapter is indistinguishable from no-TTA, which indicates that the gain comes from offline meta-training rather than the adapter architecture alone. This strengthens the deployment trade-off: COSA+ remains a strong online robustness reference, but the Dynamic Regime Adapter can deliver competitive post-shift recovery without test-time gradient updates.

\begin{table}[!htbp]
\centering
\small
\caption{Post-shift MSE (\texttt{mse\_after\_50}) on ETTh1/DLinear/h=720, shift magnitude 3.0, including Dynamic Regime Adapter. Lower is better.}
\label{tab:blackswan-main}
